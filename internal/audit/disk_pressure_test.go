...
// pause-requests mode flips RefuseRequests at critical.
func TestDiskPressureMode_PauseRequestsRefuses503AtCritical(t *testing.T) {
	...
	st.EvaluateAndReact(context.Background(), nil, fakeDiskStat(99.0), time.Now())
	if !st.RefuseRequests() {
		t.Fatal("RefuseRequests at 99%% used in pause mode; want true")
	}
...
func TestDiskPressureMode_RotateAggressivelyDropsOldestAtCritical(t *testing.T) {
	...
	// Drive a critical-state tick. Because GetDiskStatus reads the
	// real filesystem (we can't fake it for the inner drop-loop),
	// the dropOldestArchives loop will likely "exit early" — but
	// the SIDE-EFFECT we care about is that rotate-aggressively
	// NEVER flips refuse_requests, and the mode-handling code runs.
	st.EvaluateAndReact(context.Background(), nil, fakeDiskStat(96.0), time.Now())
	if st.RefuseRequests() {
...
func TestDiskPressureTransition_EmitsAdminActionOCSF(t *testing.T) {
	...
	// ok → critical: transition, one event.
	st.EvaluateAndReact(context.Background(), emitter, fakeDiskStat(96.0), time.Now())
	...
	// critical → emergency: transition, one more event.
	st.EvaluateAndReact(context.Background(), emitter, fakeDiskStat(99.0), time.Now())
...
func TestStopOnDiskCriticalAliasEquivalentToPauseMode(t *testing.T) {
	...
	longForm.EvaluateAndReact(context.Background(), nil, fakeDiskStat(96.0), time.Now())
	aliasState.EvaluateAndReact(context.Background(), nil, fakeDiskStat(96.0), time.Now())
